...
main = tkinter.Tk()
main.title("Toward detection and attribution of cyber-attacks in IOT-enabled cyber-physical systems") #designing main screen
...
def attackAttributeDetection():
    ...
    for i in range(len(predict)):
        if predict[i] == 0:
            ...
            text.insert(END,"New Test Data : "+str(values[i])+" ====> CYBER ATTACK DETECTED & Identified As : "+str(labels[predict[i]])+"\n\n")                       
...
title.config(bg='greenyellow', fg='black')  
...
autoButton = Button(main, text="AutoEncoder", command=runAutoEncoder)
autoButton.place(x=500,y=550)
autoButton.config(font=font1)

dtButton = Button(main, text="Decision Tree with PCA", command=runDecisionTree)
dtButton.place(x=700,y=550)
dtButton.config(font=font1)

dnnButton = Button(main, text="Proposed DNN model", command=runDNN)
dnnButton.place(x=950,y=550)
...
tableButton = Button(main, text="Performance Comparison", command=comparisonTable)
tableButton.place(x=630,y=650)
